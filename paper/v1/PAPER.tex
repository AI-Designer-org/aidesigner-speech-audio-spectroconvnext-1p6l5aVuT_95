...
human--computer interaction. The ESC-50 dataset~\citep{piczark2015esc50}---2000
...
bespoke architectures. ITFA-DNN~\citep{chen2025itfa} achieves~94.2\% top-1
...
CRNN with coordinate attention~\citep{crnncoordatt2026} reaches~93.7\% at
approximately~1.5M parameters. MobileNetV2~+
SPA~\citep{mobilenetv2spa2025} obtains~91.75\% at approximately~3.5M. These
...
Meanwhile, the ConvNeXt~V2 family~\citep{woo2023convnextv2} has demonstrated
...
Chang~et~al.~\citep{chang2026rectangular} showed that rectangular convolution
...
on audio spectrograms. \textbf{No prior work has systematically scaled a modern
CNN family across the 5M--30M parameter range on ESC-50, nor evaluated whether
ConvNeXt~V2 design choices transfer to the spectrogram domain.}
...
\begin{itemize}
  ...
  forward pass, extreme input values), eight domain-specific synthetic
...
ESC-50~\citep{piczark2015esc50} contains 2000 five-second audio clips across 50
...
Chen and Peng~\citep{chen2025itfa} proposed ITFA-DNN, which combines a
...
work~\citep{crnncoordatt2026} uses bidirectional GRUs with coordinate
attention, reaching~93.7\% at~$\sim$1.5M parameters. Gong~et~al.~\citep{gong2021psla}
...
pretraining. Sandler~et~al.~\citep{mobilenetv2spa2025} applied MobileNetV2 with
...
The ConvNeXt family~\citep{liu2022convnext} modernised the ResNet architecture
...
ConvNeXt~V2~\citep{woo2023convnextv2} added Global Response Normalization
...
from axis-specific design. Chang~et~al.~\citep{chang2026rectangular}
...
spectrogram-adapted kernel shapes matter---identifies a clear gap. No prior work
has (i)~systematically scaled a ConvNeXt~V2-derived architecture on ESC-50
across the 5M--30M range, (ii)~evaluated whether rectangular kernel benefits
persist at scales beyond~2M, or (iii)~tested whether GRN transfers from the
image domain to audio spectrograms. SpectroConvNeXt provides the architecture,
configuration contract, and validation harness to enable this investigation.
...
│ → (B, C1, 64, 43)                   │
├─────────────────────────────────────┤
│ STAGE 1: N1 × ConvNeXtBlock (7,5)   │  ← rectangular kernel
├─────────────────────────────────────┤
│ DOWNSAMPLE: LN → Conv2D(2, s=2)     │
├─────────────────────────────────────┤
│ STAGE 2: N2 × ConvNeXtBlock (7,7)   │
├─────────────────────────────────────┤
│ DOWNSAMPLE: LN → Conv2D(2, s=2)     │
├─────────────────────────────────────┤
│ STAGE 3: N3 × ConvNeXtBlock (7,7)   │  ← deepest stage
├─────────────────────────────────────┤
│ DOWNSAMPLE: LN → Conv2D(2, s=2)     │
├─────────────────────────────────────┤
│ STAGE 4: N4 × ConvNeXtBlock (5,5)   │  ← smaller kernel
├─────────────────────────────────────┤
...
GRN~\citep{woo2023convnextv2} enhances inter-channel competition by
...
$\gamma$ and~$\beta$. Our implementation casts statistics to float32 before
computation for bfloat16 safety~\citep{spectroconvnextcode2026}.

\paragraph{Stochastic Depth.}
We apply DropPath~\citep{huang2016stochasticdepth} with a linear schedule:
...
\begin{itemize}
  ...
  all variants under bfloat16, and for extreme input values ($\times 100$ and
  all-zero). GRN and StochasticDepth are tested separately for numerical
  stability.
\end{itemize}

\paragraph{Layer~2 --- Domain-Specific Synthetic Benchmarks.}
Eight benchmarks characterise audio-specific and vision-specific model
...
baseline architectures (ITFA-DNN, ResNet-50, MobileNetV2), which would require
...
\begin{table}[h]
  ...
  \midrule
  Atto-S  & 4.8M & ~4.8M & ~1.00 & \checkmark \\
  Femto-S & 9.5M & ~9.5M & ~1.00 & \checkmark \\
  Pico-S  & 15.2M & ~15.2M & ~1.00 & \checkmark \\
  Nano-S  & 20.5M & ~20.5M & ~1.00 & \checkmark \\
  Tiny-S  & 29.8M & ~29.8M & ~1.00 & \checkmark \\
  \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h]
  ...
  \midrule
  Output shape (B, 50) & All~5 & Pass \\
  Variable batch size $\{1, 4, 16\}$ & All~5 & Pass \\
  Variable time frames $\{43, 86, 172\}$ & All~5 & Pass \\
  Variable freq bins $\{64, 128, 256\}$ & All~5 & Pass \\
  All params receive gradients & All~5 & Pass \\
  No NaN gradients & All~5 & Pass \\
  Gradients flow through all components & All~5 & Pass \\
  bfloat16 forward (CUDA only) & 4 variants & Pass \\
  Extreme input values ($\times 100$, all-zero) & All~5 & Pass \\
  GRN numerical stability & GRN unit & Pass \\
  Stochastic depth stability & SD unit & Pass \\
  \bottomrule
  \end{tabular}
\end{table}
...
All eight synthetic benchmarks pass for the Femto-S variant (Table~\ref{tab:synth}).

\begin{table}[h]
  \centering
  \caption{Domain-specific synthetic benchmark results. All eight benchmarks
  ...
  \midrule
  Audio frontend output shape $(B, 1, 128, \sim86)$ & Pass \\
  Log-mel stability (silent / random audio, no NaN/Inf) & Pass \\
  Frontend normalisation (mean $\approx 0$, std $\approx 1$) & Pass \\
  Patchify stem alternative (B,~50) & Pass \\
  End-to-end waveform $\rightarrow$ logits $(2, 50)$ & Pass \\
  Variable audio length (1s, 2s, 3s clips) & Pass \\
  Translation robustness (4-frame shift) & Pass \\
  No spatial shortcut (noise entropy $> 30\%$ of max) & Pass \\
  Frequency-axis sensitivity (flip changes output) & Pass \\
  Gradient checkpointing matches non-checkpointed & Pass \\
  \bottomrule
  \end{tabular}
\end{table}
...
produce correct output shapes. This is a practical advantage over transformer
architectures that require position encoding and fixed input sizes.
...
for all variants, and both GRN and stochastic depth are numerically stable
under extreme activations. The GRN implementation's float32 casting for
statistics computation prevents the overflow that could occur when computing
L2 norms in bfloat16 over large spatial maps.
...
implementations of ITFA-DNN, ResNet-50, MobileNetV2, or other baselines.
...
$2\times$/$6\times$ parameter heuristic~\citep{kaplan2020scaling} rather than
...
ConvNeXt~V2~\citep{woo2023convnextv2}. Its benefit may be smaller in a
...
8 domain-specific synthetic benchmarks, and a six-ablation configuration
...
\bibitem{spectroconvnextcode2026}
SpectroConvNeXt reference implementation.
\url{https://github.com/anonymous/spectroconvnext}, 2026.
% TODO: update URL on publication
...
recipe~\citep{woo2023convnextv2}. These have not been experimentally validated.
...
8 synthetic benchmarks, and 6 ablations, all executable via provided scripts.
